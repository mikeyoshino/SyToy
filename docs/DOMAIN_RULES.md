...
- Product มี persisted positive `Version` เริ่มที่ 1; การ Update/Publish/Archive ที่สำเร็จเพิ่มครั้งเดียวต่อ logical command ส่วน stale, rejected และ no-op ไม่เพิ่ม version. Lifecycle ที่อนุญาตคือ `Draft -> Published -> Archived`; Draft และ Published แก้ข้อมูลสินค้าได้โดยคง lifecycle status เดิม ส่วน Archived เป็น terminal และแก้ไม่ได้ ทั้งนี้ Published Pre-order ล็อกวันปิดรอบ แต่ Admin ปรับจำนวนรับทั้งหมดก่อนปิดรอบได้โดยห้ามต่ำกว่ายอด held, committed และ retired และต้องอัปเดต Product offer, capacity กับ append-only movement evidence ใน transaction เดียวกัน
