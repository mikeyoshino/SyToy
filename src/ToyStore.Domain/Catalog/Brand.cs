...
public sealed class Brand
{
    ...
    public CatalogMediaReference? Image { get; private set; }

    public CatalogReferenceStatus Status { get; private set; }

    public bool CanBeUsedByPublishedProduct =>
        Status == CatalogReferenceStatus.Active && Image is not null;
        ...
        string actor)
    {
        ...
        return new Brand(id, displayName, englishName, slug, null, createdAtUtc, actor);
    }

    public static Brand CreateWithImage(
        Guid id,
        string displayName,
        string englishName,
        CatalogSlug slug,
        ...
        string actor)
    {
        ...
        EnsureMedia(image);
        ValidateInitialAudit(createdAtUtc, actor);
        return new Brand(id, displayName, englishName, slug, image, createdAtUtc, actor);
    }
    ...
    public void UpdateDetailsWithImage(
        string displayName,
        string englishName,
        CatalogSlug slug,
        CatalogMediaReference? replacementImage,
        ...
        string actor)
    {
        ...
        var preparedEnglishName = CatalogReferenceLimits.PrepareName(englishName);
        EnsureSlug(slug);
        var resultingImage = replacementImage ?? Image;
        EnsureMedia(resultingImage);
        ValidateAudit(changedAtUtc, actor);

        if (HasSameDetails(preparedDisplayName, preparedEnglishName, slug)
            && Equals(Image, resultingImage))
        {
            return;
        }
    ...
    public void AttachOrReplaceImage(
        CatalogMediaReference image,
        DateTimeOffset changedAtUtc,
        string actor)
    {
    ...
    private static void EnsureMedia(CatalogMediaReference? image)
    {
        if (image is null)
        {
