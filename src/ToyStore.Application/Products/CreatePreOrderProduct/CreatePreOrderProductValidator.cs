...
public sealed class CreatePreOrderProductValidator : AbstractValidator<CreatePreOrderProductCommand>
{
    public CreatePreOrderProductValidator(TimeProvider timeProvider)
    {
        ...
        RuleFor(x => x.EstimatedArrivalMonth).InclusiveBetween(1, 12).WithMessage("เดือนที่สินค้าคาดว่าจะมาถึงไม่ถูกต้อง");
        RuleFor(x => x.EstimatedArrivalYear).InclusiveBetween(1, 9999).WithMessage("ปีที่สินค้าคาดว่าจะมาถึงไม่ถูกต้อง");
        ...
        RuleFor(x => x.MaxPerCustomer).GreaterThan(0).WithMessage("จำนวนสูงสุดต่อคนต้องมากกว่า 0")
            .LessThanOrEqualTo(x => x.TotalCapacity).WithMessage("จำนวนสูงสุดต่อคนต้องไม่เกินจำนวนรับพรีออเดอร์");
        RuleFor(x => x.BalancePaymentDays).GreaterThan(0).WithMessage("ระยะเวลาชำระยอดคงเหลือต้องมากกว่า 0 วัน");
        RuleFor(x => x.Images).Custom((slots, context) => InStockProductValidationRules.ValidateMedia(slots, context, false));
